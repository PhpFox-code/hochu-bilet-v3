<?php
    ...
    class Metrika extends \Backend\Modules\Base {

        public $tpl_folder = 'SeoMetrika';
        public $tablename = 'seo_metrika';

        function before() {
            parent::before();
            $this->_seo['h1'] = 'Метрика';
            $this->_seo['title'] = 'Метрика';
            $this->setBreadcrumbs('Метрика', 'backend/'.Route::controller().'/index');
        }

        function indexAction () {
            $result = DB::select()->from($this->tablename)->order_by('id', 'ASC')->as_object()->execute();
